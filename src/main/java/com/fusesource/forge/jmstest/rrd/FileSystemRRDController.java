...
public class FileSystemRRDController extends CachingRRDController implements Releaseable {
	
	private String fileName = "jmstest.rrd"; 
	private long   step = 300L;
	private int archiveLength = 100;
	private Map<String, RRDRecorderImpl> rrdRecorder;
	...
	public FileSystemRRDController() {
	...
	synchronized public Map<String, RRDRecorderImpl> getRrdRecorder() {
		if (rrdRecorder == null) {
			rrdRecorder = new HashMap<String, RRDRecorderImpl>();
		}
		return rrdRecorder;
	}
	
	synchronized public void addRrdRecorder(RRDRecorderImpl recorder) {
	...
	synchronized public void start() throws Exception {
		...
		Collection<RRDRecorderImpl> recorders = new ArrayList<RRDRecorderImpl>();
		recorders.addAll(getRrdRecorder().values());
		getRrdRecorder().clear();
		
		for(RRDRecorderImpl recorder: recorders) {
